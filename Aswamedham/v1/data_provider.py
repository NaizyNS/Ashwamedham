...
data_set = pd.read_csv("datasets\\cricket_Test_1.csv")
                       # header=None, names=["ID", "category", "name", "country", "birth_year", "dead_or_alive", "gender"])

# for skip_blank_lines in [True, False]:
#     data_set = pd.read_csv("datasets\\cricket_Test_1.csv", quoting=csv.QUOTE_NONE, sep=',', error_bad_lines=False, skip_blank_lines=skip_blank_lines)


user_data = {
    const.gender_txt: "",
    const.dead_or_alive_txt: "",
    const.category_txt: "Cricket",
